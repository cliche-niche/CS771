...
numTest = 500
START  = 0
filepaths = [ f"test/{i}.png" for i in range( START, numTest + START ) ]
...
with open( "test/codes.txt", 'r' ) as file:
	# use the same indices for gold_labels as for filepath
	gold_labels = file.read().splitlines()
	gold_labels = gold_labels[ START : numTest + START ]
